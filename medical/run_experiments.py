"""
对一个或多个疾病一次性跑全部 input 配置，结果累加写入 results.csv：
  HAN | icd only          | <disease>
  HAN | icd + ukb (early) | <disease>
  HAN | icd + ukb (late)  | <disease>

约定的数据目录布局（与用户云端实际结构一致）:
  <data-root>/
    datasets_agg/             dataset_{Disease}.csv   ← aggregated + UKB
    datasets_agg_no_ukb/      dataset_{Disease}.csv   ← aggregated, ICD only
    datasets_longitudinal/    dataset_{Disease}.csv   ← (HAN 不用)
    datasets_longitudinal_no_ukb/  dataset_{Disease}.csv   ← (HAN 不用)

HAN 是静态异质图模型，按 modeling_raw_plan.md §四.2 的推荐使用 aggregated。
因此本脚本只读取 datasets_agg / datasets_agg_no_ukb 两个文件夹。

每个 (disease, mode) 配置开独立子进程运行，避免 TF1 graph/session 状态串扰。

用法（单个疾病）:
  python run_experiments.py --data-root /path/to/data --diseases Cardiac_Fibrosis

用法（全部 7 个疾病）:
  python run_experiments.py --data-root /path/to/data --diseases all --reset-out
"""
from __future__ import annotations

import argparse
import os
import subprocess
import sys


ALL_DISEASES = [
    "CKD",
    "Cardiac_Fibrosis",
    "Crohns_Disease",
    "Fibrosis_of_Skin",
    "MASH",
    "Pulmonary_fibrosis",
    "SSc_Connective_Tissue",
]

# 子目录名（与用户实际目录命名对齐）
AGG_NO_UKB_DIR = "datasets_agg_no_ukb"
AGG_WITH_UKB_DIR = "datasets_agg"


def _resolve_csv(data_root: str, sub_dir: str, disease: str) -> str:
    p = os.path.join(data_root, sub_dir, f"dataset_{disease}.csv")
    if not os.path.isfile(p):
        raise FileNotFoundError(p)
    return p


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--data-root", required=True,
                    help="包含 datasets_agg / datasets_agg_no_ukb 子文件夹的根目录")
    ap.add_argument("--diseases", nargs="+", default=["all"],
                    help="疾病名列表；'all' 表示跑全部 7 个疾病")
    ap.add_argument("--out", default="results.csv")
    ap.add_argument("--epochs", type=int, default=200)
    ap.add_argument("--patience", type=int, default=30)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--reset-out", action="store_true",
                    help="先清空 --out 文件再写入（多个 disease 累加场景下用一次即可）")
    ap.add_argument("--modes", nargs="+",
                    default=["icd_only", "icd_ukb_early", "icd_ukb_late"],
                    choices=["icd_only", "icd_ukb_early", "icd_ukb_late"])
    args = ap.parse_args()

    diseases = ALL_DISEASES if args.diseases == ["all"] else args.diseases
    here = os.path.dirname(os.path.abspath(__file__))
    runner = os.path.join(here, "ex_medical.py")
    out_path = os.path.abspath(args.out)
    data_root = os.path.abspath(args.data_root)

    if args.reset_out and os.path.exists(out_path):
        os.remove(out_path)

    print(f"data_root: {data_root}")
    print(f"diseases:  {diseases}")
    print(f"modes:     {args.modes}")
    print(f"out:       {out_path}")

    for disease in diseases:
        no_ukb_csv = _resolve_csv(data_root, AGG_NO_UKB_DIR, disease)
        with_ukb_csv = _resolve_csv(data_root, AGG_WITH_UKB_DIR, disease)

        for mode in args.modes:
            csv = no_ukb_csv if mode == "icd_only" else with_ukb_csv
            cmd = [
                sys.executable, runner,
                "--csv", csv,
                "--mode", mode,
                "--disease", disease,
                "--epochs", str(args.epochs),
                "--patience", str(args.patience),
                "--seed", str(args.seed),
                "--out", out_path,
            ]
            print("\n>> " + " ".join(cmd))
            subprocess.run(cmd, check=True)

    print(f"\nAll runs finished. Results: {out_path}")


if __name__ == "__main__":
    main()
